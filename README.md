...
### 5. セキュリティグループのパブリック開放禁止（SSM による自律修復）
* **設計**: インフラ運用上の事故を防ぐため、SSH（ポート22）や RDP（ポート3389）の通信をインターネット全体（`0.0.0.0/0`）に開放するセキュリティグループルールの作成を禁止しています。
* **自律修復 (Self-Healing)**: 万が一誤設定によってこれらのパブリック開放ルールがデプロイされた場合、プラットフォーム側で稼働する **AWS Config ＆ Systems Manager (SSM) Automation** がこれをリアルタイムに自動検知し、数秒以内にそのルールを強制的に削除・修復する強固なポリシー統制を敷いています。

### 6. コンテナの Root 実行禁止
* **設計**: EKS上の各ワークロードおよびポッド定義において、コンテナが Root（管理者）権限で実行されることを禁止しています。
* **実装**: コンテナが非 Root ユーザー（User 設定または Kubernetes 内の `securityContext`）で起動される設定を必須化し、コンテナブレイクアウトやホスト侵害のリスクを最小限に抑えています。

### 7. CloudFront VPC Origins による本番/検証環境の完全閉域化 (stg/prodのみ)
* **設計**: 本番（`prod`）および検証（`stg`）環境において、オリジンとなる Application Load Balancer (ALB) をパブリックインターネットから完全に隠蔽するため、**CloudFront VPC Origins** を採用しています。
* **効果**: ALB をプライベート（またはアイソレート）サブネットに配置し、パブリック IP を一切排除した「Internal ALB」として動作させます。CloudFront は専用の VPC Origin コネクタを介して VPC 内部の ALB に直接接続するため、インターネットからの直接の迂回アクセス（直撃）が物理的に不可能な極めて堅牢なゼロトラスト境界を実現します。
...
terraform init \
  -backend-config="bucket=aws-landing-zone-<env>-tfstate-<AWSアカウントID>" \
...
### 3. Resource Scheduler による開発環境の夜間自動停止（FinOps コスト最適化）
* **設計**: 開発環境（`dev`）の不要な稼働コストを削減するため、RDS データベースリソース（クラスターおよびインスタンス）に対し、`Schedule = "office-hours"` タグを付与しています。
* **効果**: プラットフォーム（Landing Zone）側に構築された共通の自動停止・起動システムがこのタグを検知し、毎日 JST 20:00 に自動停止、翌朝 JST 08:00 に自動起動します。非稼働時間（夜間・休日）の不要な料金を削減し、開発環境単体で **約 60% 以上の RDS コスト削減** を実現する実務的な設計になっています。

### 4. 環境パリティと開発効率の両立 (VPC Origins ハイブリッドアーキテクチャ)
* **設計 (stg/prod)**: 検証および本番環境では、セキュリティを極限まで高めるため、ALB をプライベートサブネットに配置してインターネットからのインバウンドを遮断し、**CloudFront + VPC Origins** 経由のアクセス経路のみに限定します。
* **設計 (dev)**: 開発環境では、CloudFront の追加運用コスト（VPCコネクタ接続時間料金等）を削減するとともに、開発者が ALB に直接アクセスして迅速な動作検証（DXの向上）を行えるよう、パブリック ALB 構成を維持します。
* **実装方式**: Terraform の環境名（`var.env_name`）による動的切り替えロジックを実装し、コードの一元化と各環境レベルに適したコスト・セキュリティの最適化を両立しています。
...
本プロジェクトは、AWS Landing Zone (AFT) 移行に対応し、環境別に S3 ステートバケットを動的に切り替えてデプロイする CD パイプライン（`iac-cd.yml`）を標準搭載しています。
* **IAMロール認証 (OIDC)**: 静的なアクセスキーを排除し、プラットフォームが提供するデプロイロール `arn:aws:iam::<AWSアカウントID>:role/GitHubActionsEKSDeployRole` を OIDC 連携により一時的に引き受けるセキュアな設計です。
* **OIDC ブランチ制限**: 本番環境（`prod`）のデプロイ用ロールに対しては、プラットフォーム側のセキュリティポリシーにより `main` ブランチからの接続のみを厳密に許可するガードレールを敷いています（現在はポートフォリオ用に検証モードで動作します）。
